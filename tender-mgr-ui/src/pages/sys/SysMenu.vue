            ...
            <Form ref="formItem" :model="formItem" :label-width="80" >
              <FormItem label="父级菜单"  prop="parentId">
                <Select v-model="formItem.parentId" :disabled="editFlag">
                  <Option v-for="(item,index) in parentMenuList" v-bind:key="item.id" :value="item.id">{{ item.name }}
                  </Option>
                </Select>
              </FormItem>
              ...
              <FormItem label="图标" prop="icons">
                <Input v-model="formItem.icons" placeholder="图标class"></Input>
              </FormItem>
  ...
  export default {
    ...
    methods: {
      onSelectChange() {
        ...
        this.$http.get(api.SYS_MENU_ID + "/" + id).then(res => {
          let result = res.data.data;
          if(result.icon!=null){
            this.formItem.icons = result.icon.substr(result.icon.indexOf("-") + 1,result.icon.length);
          }else{
            this.formItem.icons=null;
          }
      ...
      edit() {
        this.formItem.icon="iconfont icon-"+this.formItem.icons;
        this.$http.post(api.SYS_MENU_EDIT, this.formItem).then(res => {
      ...
      add() {
        this.$http.post(api.SYS_MENU_ADD, this.formItem).then(res => {
          Message.message(res.data);
          //将放置在state中的树形菜单进行更新
          this.getTreeMenu();
          this.getMenuList();
      ...
      del() {
        ...
        this.$http.get(api.SYS_MENU_DELETE + "/" + delId).then(res => {
          Message.message(res.data);
          this.getTreeMenu();
          this.getMenuList();
        })
      ...
      getTreeMenu() {
        //获取树菜单
        this.$http.get(api.SYS_MENU_TREE).then(res => {
          this.data2 = res.data.data;
        });
      },
      getMenuList() {
        //获取菜单列表
        this.$http.get(api.SYS_MENU_SELECT).then(res => {
